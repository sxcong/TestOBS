...
void ScreenshotObj::Copy()
{
	...
	if (gs_stagesurface_map(stagesurf, &videoData, &videoLinesize)) {
		...
		} else 
		{
			...
			{
				/*std::vector<uint8_t> pixels(cx * cy * 4);
				uint8_t* src = static_cast<uint8_t*>(videoData);
				uint32_t srcRowPitch = videoLinesize;

				// 注意：DXGI_FORMAT_R8G8B8A8_UNORM 是 RGBA，但内存中是 BGRA（小端）
				// 实际上，在 Windows 上，R8G8B8A8_UNORM 存储为 RGBA 字节顺序
				// 但为了保险，我们按 RGBA 读取，stb_write_bmp 会转成 BGR

				for (uint32_t y = 0; y < cy; ++y) {
					memcpy(pixels.data() + y * cx * 4,
						src + y * srcRowPitch,
						cx * 4);
				}*/

				bool success = stbi_write_bmp("e:\\text.bmp", cx, cy, 4, videoData);
				printf("%d", success);
			}
